...
function addCompra(req, res){
              ...
              const sqlInsertCaja = 'INSERT INTO Compras (Monto, FechaYHora, MetodoPago, NumeroTarjeta, Puntaje, IDArticulo, IDUsuario, IDDireccion) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
              const values = [Monto, FechaYHora, MetodoPago, NumeroTarjeta, Puntaje, IDArticulo, IDUsuario, IDDireccion];
              connection.query(sqlInsertCaja, values, (err, result) => {
                  if (err) {
                    console.error(err);
...
function updateCompra(req, res){
    ...
    const sqlInsertCaja = 'UPDATE Compras SET Monto = ?, MetodoPago = ?, NumeroTarjeta = ?, Puntaje = ?, IDArticulo = ?, IDUsuario = ?, IDDireccion = ? WHERE ID = ?';
